...
def _process_uploaded_logs(
    session_state: object,
    uploaded_files: list[UploadedLogFile],
) -> None:
    ...
    if not uploaded_files:
        ...
        session_state["processing_summary"] = ProcessingSummary(
            status_message="No files uploaded."
        )
        return
...
def main() -> None:
    ...
    st.caption(
        "Upload log files, index them, and ask questions using the existing backend."
    )
    ...
    if not st.session_state["logs_indexed"]:
        st.warning(
            "Upload logs and click Process Logs to index them before starting chat."
        )
        return

    user_question = st.text_input("Ask a question about the logs")

    analysis_type_label, top_k, run_analysis = render_chat_controls(
    ...
    if run_analysis:
        ...
        try:
            response = analysis_service.analyze(
                ...
                analysis_type=_analysis_type_from_label(
                    analysis_type_label
                ),
                top_k=top_k,
            )
            ...
            if "GEMINI_API_KEY" in error_message:
                st.error(
                    "Gemini is unavailable. Add GEMINI_API_KEY to your environment and try again."
                )
            else:
